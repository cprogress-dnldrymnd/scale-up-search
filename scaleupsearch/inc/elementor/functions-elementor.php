<?php
if ( ! function_exists( 'ogeko_elementor_get_render_attribute_string' ) ) {
	function ogeko_elementor_get_render_attribute_string($element, $obj) {
		return $obj->get_render_attribute_string($element);
	}
}
if ( ! function_exists( 'ogeko_elementor_parse_text_editor' ) ) {
	function ogeko_elementor_parse_text_editor( $content, $obj ) {
...
if ( ! function_exists( 'ogeko_elementor_get_strftime' ) ) {
	function ogeko_elementor_get_strftime( $instance, $obj ) {
		$string = '';
...
if (!function_exists('ogeko_elementor_breakpoints')) {
    function ogeko_elementor_breakpoints() {
        ...
        foreach (array_reverse($breakpoints) as $breakpoint) {
            if ($breakpoint->is_enabled()) {
                ...
                for ($i = 1; $i <= 8; $i++) {
                    ...
                    $var .= 'body.theme-ogeko [data-elementor-columns-'.$device_name.'="'.$i.'"] .column-item{flex: 0 0 '.$ratio.'%; max-width: '.$ratio.'%;}';
                }
                $var .='}';
            }
        }

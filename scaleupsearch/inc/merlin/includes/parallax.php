...
class Ogeko_Elementor_Parallax {
    public function __construct() {
        ...
        add_action('elementor/frontend/section/after_render', [$this, 'ogeko_paralax_render'], 10, 1);
    }
    ...
    public function ogeko_paralax_render($element) {
        $svgfile = $this->render_svg();
        wp_enqueue_script('tweenmax');
        wp_enqueue_script('parallaxmouse');
        $settings      = $element->get_settings();
        $repeater_list = (isset($settings['ogeko_section_parallax_layer']) && $settings['ogeko_section_parallax_layer']) ? $settings['ogeko_section_parallax_layer'] : array();
        if ($settings['ogeko_section_parallax_switcher'] === 'yes') {
            foreach ($repeater_list as $layer):

                $html = '';
                if ($layer['ogeko_section_parallax_item_style'] == 'image') {
                    $image_url = $layer['ogeko_section_parallax_image']['url'];
                    if (!empty($image_url)) {
                        $path_parts = pathinfo($image_url);
                        if ($path_parts['extension'] === 'svg') {
                            $image = $layer['ogeko_section_parallax_image'];
                            if ($image['id']) {
                                $pathSvg = get_attached_file($image['id']);
                                $html    = ogeko_get_icon_svg($pathSvg);
                            }

                        } else {
                            $html = '<img src="' . $layer['ogeko_section_parallax_image']['url'] . '" title="" alt="">';
                        }
                    }

                } else {
                    $html = $svgfile[$layer['ogeko_section_parallax_item_svg_style']];
                }
                $parallax_mouse = $layer['ogeko_section_parallax_mouse'] == 'yes' ? 'true' : 'false';
                echo '<div class="parallax-layer elementor-repeater-item-' . $layer['_id'] . ' parallax-layer-' . $layer['_id'] . $element->get_id() . ' f-' . $layer['ogeko_section_parallax_item_svg_color_type'] . '" data-parallax="' . $parallax_mouse . '" data-rate="' . $layer['ogeko_section_parallax_rate'] . '">' . $html . '</div>';
            endforeach;
            ...
            <script type="text/javascript">
                (function ($) {
                    ...
                    $(' .parallax-layer-<?php echo $layer['_id'] . $element->get_id();?>').prependTo($('.elementor-element-<?php echo $element->get_id(); ?>')).css({
                        'z-index': <?php echo !empty($layer['ogeko_section_parallax_z_index']) ? $layer['ogeko_section_parallax_z_index'] : 0; ?>,
                        '<?php echo is_rtl() ? 'right' : 'left';?>': <?php echo !empty($layer['ogeko_section_parallax_position_h']['size']) ? $layer['ogeko_section_parallax_position_h']['size'] : 50; ?> +'<?php echo !empty($layer['ogeko_section_parallax_position_h']['unit']) ? $layer['ogeko_section_parallax_position_h']['unit'] : '%'; ?>',
                        'top': <?php echo !empty($layer['ogeko_section_parallax_position_v']['size']) ? $layer['ogeko_section_parallax_position_v']['size'] : 50; ?> +'<?php echo !empty($layer['ogeko_section_parallax_position_v']['unit']) ? $layer['ogeko_section_parallax_position_v']['unit'] : '%'; ?>'
                    });
    ...
    public function register_controls($element, $args) {
        ...
        $element->start_controls_section(
            'ogeko_section_parallax',
            [
                'label' => __('Multi Parallax ', 'ogeko'),
                'tab'   => Controls_Manager::TAB_LAYOUT,
            ]
        );
        $element->add_control('ogeko_section_parallax_switcher',
            [
                'label' => __('Enable Parallax', 'ogeko'),
        ...
        $repeater->add_control(
            'ogeko_section_parallax_item_style',
            [
                'label'   => __('Parallax Type', 'ogeko'),
                'type'    => Controls_Manager::SELECT,
                'options' => [
                    'image' => __('Image', 'ogeko'),
                    'svg'   => __('Svg', 'ogeko'),
                    'svg'   => __('Opal Style', 'ogeko'),
                ],
                'default' => 'image',
            ]
        );
        ...
        $repeater->add_control(
            'ogeko_section_parallax_item_svg_style',
            [
                'label'     => __('Opal Style', 'ogeko'),
                'type'      => Controls_Manager::SELECT,
                'options'   => [
                    '0' => __('Style 1', 'ogeko'),
                    '1' => __('Style 2', 'ogeko'),
                ...
                'default'   => '1',
                'condition' => [
                    'ogeko_section_parallax_item_style' => 'svg'
                ]
            ]
        );

        $repeater->add_control(
            'ogeko_section_parallax_image',
            [
                ...
                'condition' => [
                    'ogeko_section_parallax_item_style' => 'image'
                ]
            ]
        );

        $repeater->add_control(
            'ogeko_section_parallax_item_svg_color_type',
            [
                'label'   => __('Opal Style Color', 'ogeko'),
                'type'    => Controls_Manager::SELECT,
                'options' => [
                    'primary'   => __('Primary', 'ogeko'),
                    'secondary' => __('Secondary', 'ogeko'),
                    'custom'    => __('Custom', 'ogeko'),
                ],
                'default' => 'custom',
            ]
        );
        ...
        $repeater->add_control(
            'ogeko_section_parallax_item_svg_color',
            [
                'label'     => __('Custom Color', 'ogeko'),
                'type'      => Controls_Manager::COLOR,
                'selectors' => [
                    '{{WRAPPER}} {{CURRENT_ITEM}}.parallax-layer svg' => 'fill: {{VALUE}};',
                ],
                'condition' => [
                    'ogeko_section_parallax_item_svg_color_type' => 'custom',
                ],
            ]
        );

        $repeater->add_control(
            'ogeko_section_parallax_item_svg_width',

            [
                'name'       => 'ogeko_section_parallax_item_svg_width',
        ...
        $repeater->add_control(
            'ogeko_section_parallax_item_svg_height',
            [
                'label'      => __('Height', 'ogeko'),
                'type'       => Controls_Manager::SLIDER,
                'default'    => [
                    'size' => 100,
                    'unit' => '%',
        ...
        $repeater->add_control(
            'ogeko_section_parallax_item_svg_rotate',
            [
        ...
        $repeater->add_control(
            'ogeko_section_parallax_position_h',
            [
                'label'      => __('Position h', 'ogeko'),
                'type'       => Controls_Manager::SLIDER,
                'range'      => [
                    '%'  => [
                        'min' => 0,
        ...
        $repeater->add_control(
            'ogeko_section_parallax_position_v',
            [
                'label'      => __('Position v', 'ogeko'),
                'type'       => Controls_Manager::SLIDER,
                'range'      => [
                    '%'  => [
                        'min' => 0,
        ...
        $repeater->add_control(
            'ogeko_section_parallax_z_index',
            [
                'label'       => __('Z-Index', 'ogeko'),
                'type'        => Controls_Manager::NUMBER,
                'default'     => 1,
                'description' => __('Choose z-index for the parallax layer, default: 3', 'ogeko'),
            ]
        );
        $repeater->add_control(
            'ogeko_section_parallax_mouse',
            [
                'label'       => __('Mousemove Parallax', 'ogeko'),
                'type'        => Controls_Manager::SWITCHER,
                'description' => __('Enable or disable mousemove interaction', 'ogeko'),
            ]
        );
        $repeater->add_control(
            'ogeko_section_parallax_rate',
            [
                'label'   => __('Rate', 'ogeko'),
                'type'    => Controls_Manager::NUMBER,
                'default' => -10,
        ...
        $element->add_control(
            'ogeko_section_parallax_layer',
            [
                'label'     => __('Parallax Items', 'ogeko'),
                'type'      => Controls_Manager::REPEATER,
                'condition' => [
                    'ogeko_section_parallax_switcher' => 'yes',
                ],

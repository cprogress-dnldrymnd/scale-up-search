 ...
 */
class Ogeko_Megamenu_Walker extends Walker_Nav_Menu {
    ...
    public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
		...
		if (isset($item->mega_data) && !empty($item->mega_data)) {
			if (!empty($item->mega_data['icon'])) {
				...
				$icon = ogeko_megamenu_get_icon_html($icon_class, $item->mega_data);
			}
		}
    ...
    public function display_element($element, &$children_elements, $max_depth, $depth, $args, &$output) {
        ...
        $element->mega_data = ogeko_megamenu_get_item_data($element->ID);
    ...
    public function render_megamenu_elementor($item, $args, $depth) {
        ...
        if (class_exists('Elementor\Plugin')) {
            $post_id = ogeko_megamenu_get_post_related_menu($item->ID);
